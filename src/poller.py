...
def get_all_cardata(vehicle):
    endpoints = {'charge_state', 'climate_state', 'drive_state', 'gui_settings', 'vehicle_state', 'vehicle_config'}
    data = {}
    ...
    for endpoint in endpoints:
        try:
            ...
            warning_msg('Could not get data from Tesla with vin {}: {}'.format(vehicle['vin'], str(err)))
...
def poller(vehicle, last_time_sleepy, mongo_db):
    if last_time_sleepy > 0:
        ...
        if seconds_sleepy < 3600:
            ...
            info_msg('Tesla with vin {} is sleepy, but still online for {} minutes'.format(vehicle['vin'], minutes))
            return last_time_sleepy
        else:
            info_msg('Tesla with vin {} is 60 minutes sleepy, we start polling it again'.format(vehicle['vin']))
            last_time_sleepy = 0
    else:
        try:
            ...
            """
            if sleepy(car_data['drive_state']) is True and charge_state['charging_state'] != 'Charging':
...
def main():
    ...
    # Get the config file
    configfile = None
    for filename in ['/usr/local/tesladata/config.yaml', './src/config.yaml', './config.yaml']:
    ...
    while True:
        ...
        if vehicle['state'] == 'online':
            ...
            warning_msg('Tesla with vin {} has state {}. This state is unknown to us'.format(vehicle['vin'], vehicle['state']))
            last_time_sleepy = 0
